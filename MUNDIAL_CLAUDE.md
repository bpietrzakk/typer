...
```
mundial-frontend/
├── CLAUDE.md                       # ten sam plik lub osobny dla frontendu
├── README.md
...
## O autorze i współpracy

**Bartek** — zna backend (FastAPI, SQL, Python), budował projekt zaliczeniowy `typer-ligowy` o podobnej architekturze. Rozumie warstwy domenowe, migracje SQL, connection pool. Mniej doświadczony w React i frontendzie.

**Kolega** — zna React i JavaScript, ma doświadczenie w projektach z Reactem. Nie zna Pythona/FastAPI — tłumacz mu rzeczy backendowe prosto.

**Projekt referencyjny:** `typer-ligowy` — aplikacja do typowania Ekstraklasy zbudowana razem z Claude. Stamtąd można przenieść 1:1: `domain/scoring.py`, `domain/match_results.py`, `domain/predictions.py`, wzorzec `db/connection.py`, wzorzec `db/queries.py`.

---

## Workflow z Claude Code

- **Plan mode** (`Shift+Tab` dwa razy) dla każdego zadania które dotyka > 1 pliku. Bartek czyta plan i poprawia zanim zatwierdzi.
...
- **Po każdym większym kawałku wytłumacz diff** — czemu tak, jakie były alternatywy, jakie kompromisy. Bartek uczy się przez rozumienie decyzji, nie przez samo czytanie kodu.
- Jeśli Bartek pyta "co to robi" lub "jak to działa" — tłumacz prosto, analogiami, bez żargonu. Nie zakładaj że wie co to np. `httpOnly cookie` — wyjaśnij.
